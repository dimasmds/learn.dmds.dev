...
import { validateStepAnswer, StepValidator } from '@/lib/applications/services/StepValidator';
...
describe('StepValidator class', () => {
  const validator = new StepValidator();

  it('delegates to validateStepAnswer via validate method', () => {
    const step = createStep({ type: 'theory', solution: {} });
    const result = validator.validate(step, null);
    expect(result.isCorrect).toBe(true);
  });
  ...
  it('validates multiple-choice correctly', () => {
    const step = createStep({
      type: 'multiple-choice',
      content: { options: ['A', 'B'] },
      solution: { correctAnswer: 'A' },
    });
    const result = validator.validate(step, 'A');
    expect(result.isCorrect).toBe(true);
  });
});
